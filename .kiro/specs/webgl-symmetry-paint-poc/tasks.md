...
- [x] 6. 入力処理システムの実装と接続
  ...
  - [ ] 6.4 対称変換実装の修正
    ...
    - `create8AxisSymmetryTransform`関数の回転行列を反射行列に修正
    - `createRotationMatrix(angle)` → `createReflectionMatrix(angle)`に変更
    - 対称変換のコメントを「回転」から「反射」に修正
    - 対称変換テスト（transform.test.ts）の 3 つの失敗テストを修正確認
    - _Requirements: 2.2, 2.3, 2.5_

    **達成条件:**

    - ✅ `create8AxisSymmetryTransform`で反射行列を使用する
    - ✅ 点(612,512) → (412,512)の反射変換が正しく動作する
    - ✅ `src/symmetry/transform.test.ts`の 3 つのテストがパスする
    - ✅ 8 軸対称の数学的正確性が確認される
    - ✅ 反射対称の性質（2 回適用で元に戻る）が満たされる

  - [ ] 6.5 筆圧とスムージング処理（高度な機能）
